...
- 2026-08-29: Radio.co schedule widget LIVE — user provided widget ID es54da380. NOTE: the official embed script uses last-script-tag DOM insertion which breaks under React dynamic injection, so the iframe (embed.radio.co/embeds/schedule/es54da380.html) is embedded directly with the official postMessage auto-height protocol replicated. Verified rendering real schedule data with ON AIR badge
